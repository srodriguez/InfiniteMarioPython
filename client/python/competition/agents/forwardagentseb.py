...
__author__ = "Sebastian Rodriguez - Modified from original code of Sergey Karakovskiy, sergey at idsia fullstop ch"
...
#
class SebForwardAgent(MarioAgent):
    ...
    def _a2(self):
        ...
        if self.mayMarioJump:
            print("m: %d, %s, %s, 12: %d, 13: %d, j: %d") % (
                ...
                self.mayMarioJump,
                self.isMarioOnGround,
                self.levelScene[11, 12],
                self.levelScene[11, 12],
                self.trueJumpCounter,
            )
        else:
        ...
        if self.levelScene[11, 12] != 0 or self.levelScene[11, 13] != 0 or danger:
